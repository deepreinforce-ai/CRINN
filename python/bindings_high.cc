...
#include "glass/searcher/searcher_factory_high_dim.hpp"
...
struct Searcher {
  ...
  Searcher(Graph &graph, py::object input, const std::string &metric,
      ...
      : searcher(
            std::unique_ptr<glass::GraphSearcherBase>(glass::create_searcher_high_dim(
...
PYBIND11_PLUGIN(crinn_high) {
  py::module m("crinn_high");
